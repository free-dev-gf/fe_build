// borrowed from https://xjl271314.github.io/docs/project/autoTryCatch.html
...
function generateTryCatch(path, filename) {
    try {
        ...
        if (
            (blockStatement.body &&
                types.isTryStatement(blockStatement.body[0])) ||
            (!types.isBlockStatement(blockStatement) &&
                !types.isExpressionStatement(blockStatement)) ||
        ...
        var commentsNode =
            blockStatement.body.length > 0
                ...
                : blockStatement.innerComments ||
                  blockStatement.trailingComments;
        if (
            commentsNode &&
            commentsNode[0].value.indexOf(DISABLE_COMMENT) > -1
        ) {
            path.skip();
        }

        // 将catch handler转为AST节点 然后从AST节点中获取函数体 作为catch里面的内容
        var catchStatement = template.statement(
            ...
            types.variableDeclarator(
                ...
                types.ObjectExpression([
                    types.objectProperty(
                        types.identifier("line"),
                        types.numericLiteral(loc.start.line)
                    ),
                    types.objectProperty(
                        types.identifier("row"),
                        types.numericLiteral(loc.start.column)
                    ),
                    types.objectProperty(
                        types.identifier("function"),
                        types.stringLiteral(funcName)
                    ),
                    types.objectProperty(
                        types.identifier("filename"),
                        types.stringLiteral(getFilename(filename))
                    ),
                ])
            ),
        ]);
...
var visitor = {
    "ArrowFunctionExpression|FunctionExpression|FunctionDeclaration": function(
        path,
        state
    ) {
